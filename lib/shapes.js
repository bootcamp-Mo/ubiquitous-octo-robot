

// todo: Implement the Shape classes: Create the Circle, Triangle, and Square classes. You can use inheritance to place any common functionality and properties shared by the Shape classes in a parent Shape class.
class Shape {
	constructor(font, colorText, colorShape, size, text) {
		this.font = font
		this.colorText = colorText
		this.colorShape = colorShape
		this.size = size
		this.text = text
	}

	render() {

	}
}
class Circle extends Shape {
	// circle-specific properties and methods
	constructor(radius, font, colorText, colorShape, size, text) {
		super(font, colorText, colorShape, size, text);
		this.radius = radius;

	}

	get area() {
		return Math.PI * this.radius * this.radius;
	}
	render(selectedFontFamily, selectedFontSize) {
		const svg = `<svg viewBox="0 0 ${2 * this.radius} ${2 * this.radius}" xmlns="http://www.w3.org/2000/svg">
		<circle cx="${this.radius}" cy="${this.radius}" r="${this.radius}" fill="${this.colorShape}" />
		<text 
		x="${this.radius}" 
		y="${this.radius}" 
		text-anchor="middle" 
		font-family="${selectedFontFamily}" 
		font-size="${selectedFontSize}" 
		fill="${this.colorText}">${this.text}</text>
	  </svg>`;
		return svg;
	}
}
module.exports.Circle = Circle;

class Triangle extends Shape {
	// triangle-specific properties and methods
	constructor(side1, side2, side3, font, colorText, colorShape, size,) {
		super(font, colorText, colorShape, size,);
		this.side1 = side1;
		this.side2 = side2;
		this.side3 = side3;
	}
	get area() {
		return 0.5 * (this.side1 + this.side2 + this.side3);
	}
	render(selectedFontFamily, selectedFontSize) {
		const svg =
			`<svg 
		viewBox="0 0 ${2 * this.radius} ${2 * this.radius}" 
		xmlns="http://www.w3.org/2000/svg">
		  	<circle 
			cx="${this.radius}" 
			cy="${this.radius}" 
			r="${this.radius}" 
			fill="${this.colorShape}" 
			/>

			<text 
			x="${this.radius}" 
			y="${this.radius}" 
			text-anchor="middle" 
			font-family="${selectedFontFamily}" 
			font-size="${selectedFontSize}" 
			fill="${this.colorText}">${this.text}</text>
		</svg>`;
		return svg;
	}
}
module.exports.Triangle = Triangle;

class Square extends Shape {
	// square-specific properties and methods
	constructor(side, font, colorText, colorShape, size,) {
		super(font, colorText, colorShape, size,);
		this.side = side;
	}
	get area() {
		return this.side * this.side;
	}
	render(selectedFontFamily, selectedFontSize) {
		const svg = `<svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
		<rect 
		x="${this.side}" 
		y="${this.side}" 
		width="100" 
		height="100" 
		rx="10" 
		ry="10" 
		fill="${this.colorShape}" />
		<text3
		x="${this.side + 50}" 
		y="${this.side + 60}" 
		text-anchor="middle" 
		font-family="${selectedFontFamily}" 
		font-size="${selectedFontSize}" 
		fill="${this.colorText}">${this.text}</text3>
	  </svg>`;
		return svg;
	}
}
module.exports = { Circle, Triangle, Square }